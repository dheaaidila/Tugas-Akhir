<x-app>
            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                    @php
                        $no = 1;
                    @endphp
                    @foreach ($list_ck->sortByDesc('created_at')->values() as $ck)
                        <tr>
                            <td class="text-center">{{ $no++ }}</td>
                            <td class=>{{ $ck->nama_cv }}</td>
                            <td class="text-center">
                                {{ $ck->nama_pekerjaan }}
                            </td>
                            <td class="text-center">
                                <div class="btn-group">
                                    <x-template.button.info-button url="admin/ck" id="{{ $ck->id }}" />
                                    <a href="" data-toggle="modal"
                                        data-target="#edit-data{{ $ck->id }}"class="btn btn-warning"><i
                                            class="fas fa-edit"></i></a>
                                    {{-- <x-template.button.edit-button url="admin/ck" id="{{ $ck->id }}" /> --}}
                                    <x-template.button.delete-button url="admin/ck" id="{{ $ck->id }}" />
                                </div>
                            </td>
                        </tr>
                    @endforeach
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" style="text-align:center; font-size: 25px">Tambah Data CK</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ url('admin/ck') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
    ...
    <!-- Modal Edit CK -->\
    @foreach ($list_ck as $ck)
        <div class="modal fade" id="edit-data{{ $ck->id }}">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" style="text-align:center; font-size: 25px">Edit Data CK</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ url('admin/ck', $ck->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="nama_pekerjaan" name="nama_pekerjaan"
                                            class="form-control" value="{{ $ck->nama_pekerjaan }}">
                                        @error('nama_pekerjaan')
                                            <p class="text-danger" style="font-size: 12px">* {{ $message }}</p
                                                style="font-size: 12px">
                                        @enderror
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="number" id="jumlah_uang" name="jumlah_uang"
                                            class="form-control" value="{{ $ck->jumlah_uang }}">
                                        @error('jumlah_uang')
                                            <p class="text-danger" style="font-size: 12px">* {{ $message }}</p
                                                style="font-size: 12px">
                                        @enderror
